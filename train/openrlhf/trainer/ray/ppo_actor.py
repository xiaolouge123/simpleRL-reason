...
from openrlhf.trainer import PPOTrainer, PPOTrainerPRM800K_BOX
from openrlhf.trainer.ppo_utils import Experience, RemoteExperienceMaker, RemoteExperienceMakerBOX, RemoteExperienceMakerPRMBOX, RemoteExperienceMakerORMBOX
...
def preprocess_data_action(data, input_template=None, input_key="input", apply_chat_template=None) -> str:
    pass


class PromptDatasetAction(Dataset):
    """
    Dataset for PPO model with image action task
    """
    def __init__(
        self,
        dataset,
        ...
        for data in tqdm(dataset, desc="Preprocessing data", disable=not self.strategy.is_rank_0()):
            prompt = preprocess_data_action(data, input_template, input_key, apply_chat_template)
...
class ActorPPOTrainerBOX(PPOTrainerPRM800K_BOX):
    def __init__(
        self,
        *args,
        vllm_engines: List = None,
    ...
    def _save_checkpoint(self, args, tag, client_states):
        ...
        # Save DeepSpeed format checkpoint
        self.strategy.save_ckpt(
            self.actor.model,
            os.path.join(args.ckpt_path, "_actor"),
            tag,
        ...
        #     hf_ckpt_path = os.path.join(args.ckpt_path,"_actor", tag)
        ...
        #     self.strategy.save_model(self.actor, self.tokenizer, hf_ckpt_path)
        # Save HuggingFace format checkpoint
        # if self.strategy.is_rank_0():
        #     hf_ckpt_path = os.path.join(args.ckpt_path, "_actor","hf_ckpt", tag)
        #     os.makedirs(hf_ckpt_path, exist_ok=True)
...
class ActorPPOTrainerAction(PPOTrainerPRM800K_BOX):
    def __init__(
        self,
        *args,
        vllm_engines: List = None,
...
@ray.remote(num_gpus=1)
class ActorModelRayActorBOX(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
        self._setup_distributed(strategy)
...
@ray.remote(num_gpus=1)
class ActorModelRayActorAction(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
        self._setup_distributed(strategy)
...
@ray.remote(num_gpus=1)
class ActorModelRayActorPRMBOX(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
        self._setup_distributed(strategy)
...
class ActorPPOTrainerPRMBOX(PPOTrainerPRM800K_BOX):
    def __init__(
        self,
        *args,
        vllm_engines: List = None,
        ...
        **kwargs,
        ...
        self.experience_maker = RemoteExperienceMakerPRMBOX(
            self.actor,
            self.critic,
            self.reward_model,
            self.initial_model,
...
@ray.remote(num_gpus=1)
class ActorModelRayActorORMBOX(BasePPORole):
    def init_model_from_pretrained(self, strategy: DeepspeedStrategy, pretrain):
        args = strategy.args
        self._setup_distributed(strategy)
...
class ActorPPOTrainerORMBOX(PPOTrainerPRM800K_BOX):
    def __init__(
        self,
        *args,
        vllm_engines: List = None,
        ...
        **kwargs,
        ...
        self.experience_maker = RemoteExperienceMakerORMBOX(
            self.actor,
            self.critic,
            self.reward_model,
            self.initial_model,
